...
files = Path(
    "/Users/arangolab/Library/Mobile Documents/com~apple~CloudDocs/iCloud Data/Masha Helen/Single layer JV data/20220811-1-retest8d-7_8:19:22.txt"
)
...
def block_Diode(x, JoBd, b):
    return np.power(JoBd*x/d, b)			#effect of blocking layer on ideal diode
...
def Jdark_exp(x, Jo, n, JoBd, b):
    return 1/(1/Diode(x, Jo, n) + 1/block_Diode(x, JoBd, b))				#exponential leg
...
def Slope_exp(x, Jo, n, JoBd, b):
    return np.gradient(np.log10(Jdark_exp(x, Jo, n, JoBd, b)), np.log10(x))			#slope of exponential leg
def Slope_shunt(x, JoTsh, tsh, JoBsh, b):
    return np.gradient(np.log10(Jdark_shunt(x, JoTsh, tsh, JoBsh, b)), np.log10(x))	#slope of transport (shunt) leg
